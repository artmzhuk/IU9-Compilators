...
begin
 if ord(OutputCodeData[o+3])<0x80 then begin
...
begin
 ...
 while PC<CodePosition do begin
  ...
  case Opcode of
   OPAdd:begin
    OCPopEAX;
    EmitByte(0x01); EmitByte(0x04); EmitByte(0x24); (* ADD DWORD PTR {ESP},EAX *)
    LastOutputCodeValue:=locNone;
   end;
   OPNeg:begin
    OCNegDWordPtrESP;
   ...
   OPDiv2:begin
    EmitByte(0xd1); EmitByte(0x3c); EmitByte(0x24); (* SAR DWORD PTR {ESP},1 *)
    LastOutputCodeValue:=locNone;
   end;
   OPRem2:begin
    OCPopEAX;
    EmitByte(0x8b); EmitByte(0xd8); { MOV EBX,EAX }
    EmitByte(0x25); EmitByte(0x01); EmitByte(0x00); EmitByte(0x00); EmitByte(0x80); { AND EAX,0x80000001 }
    EmitByte(0x79); EmitByte(0x05); { JNS +0x05 }
    EmitByte(0x48); { DEC EAX }
    EmitByte(0x83); EmitByte(0xc8); EmitByte(0xfe); { OR EAX,BYTE -0x02 }
    EmitByte(0x40); { INC EAX }
    LastOutputCodeValue:=locNone;
    OCIMulEBX;
    OCPushEAX;
   end;
   OPEqlI:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x94); EmitByte(0xd0); // SETE AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPNEqI:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x95); EmitByte(0xd0); // SETNE AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPLssI:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x9c); EmitByte(0xd0); // SETL AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPLeqI:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x9e); EmitByte(0xd0); // SETLE AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPGtrI:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x9f); EmitByte(0xd0); // SETG AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPGEqi:begin
    ...
    OCPopEAX;
    OCCmpEAXEBX;
    EmitByte(0x0f); EmitByte(0x9d); EmitByte(0xd0); // SETGE AL
    LastOutputCodeValue:=locNone;
    OCMovzxEAXAL;
    OCPushEAX;
   end;
   OPDupl:begin
    EmitByte(0xff); EmitByte(0x34); EmitByte(0x24); // PUSH DWORD PTR [ESP]
    LastOutputCodeValue:=locNone;
   end;
   OPSwap:begin
    OCPopEBX;
    OCPopEAX;
    EmitByte(0x53); { PUSH EBX }
    LastOutputCodeValue:=locNone;
    OCPushEAX;
   end;
   OPAndB:begin
   ...
   OPCopy:begin
    ...
    OCPopESI;
    OCMovECXImm(Value);
    EmitByte(0x2b); EmitByte(0xe1); { SUB ESP,ECX }
    EmitByte(0x8b); EmitByte(0xfc); { MOV EDI,ESP }
    LastOutputCodeValue:=locNone;
    OCCLD;
    OCREPMOVSB;
    OCXChgEDXESI;
   ...
   OPAddC:begin
    if (Value>=-128) and (Value<=127) then begin
     EmitByte(0x83); EmitByte(0x04); EmitByte(0x24); EmitByte(Value); { ADD DWORD PTR [ESP],BYTE Value }
    end else begin
     EmitByte(0x81); EmitByte(0x04); EmitByte(0x24); EmitInt32(Value); { ADD DWORD PTR [ESP],DWORD Value }
    end;
    LastOutputCodeValue:=locNone;
    PC:=PC+1;
   end;
   OPMulC:begin
    if Value=(-1) then begin
     ...
     OCPopEAX;
     EmitByte(0x6b); EmitByte(0xc0); EmitByte(Value); { IMUL EAX,BYTE s }
     LastOutputCodeValue:=locNone;
     OCPushEAX;
    end else begin
     OCPopEAX;
     EmitByte(0x69); EmitByte(0xc0); EmitInt32(Value); { IMUL EAX,DWORD s }
     LastOutputCodeValue:=locNone;
     OCPushEAX;
    end;
    PC:=PC+1;
   ...
   OPJZ:begin
    ...
    OCPopEAX;
    OCTestEAXEAX;
    EmitByte(0x0f); EmitByte(0x84); { JZ Value }
    JumpTable[CountJumps]:=OutputCodeDataSize+1;
    EmitInt32(Value);
    LastOutputCodeValue:=locNone;
    PC:=PC+1;
   ...
   OPAdjS:begin
    if Value>0 then begin
     if (Value>=-128) and (Value<=127) then begin
      EmitByte(0x83); EmitByte(0xc4); EmitByte(Value); { ADD ESP,BYTE Value }
     end else begin
      EmitByte(0x81); EmitByte(0xc4); EmitInt32(Value); { ADD ESP,DWORD Value }
     end;
    end else if Value<0 then begin
     Value:=-Value;
     if (Value>=-128) and (Value<=127) then begin
      EmitByte(0x83); EmitByte(0xec); EmitByte(Value); { SUB ESP,BYTE Value }
     end else begin
      EmitByte(0x81); EmitByte(0xec); EmitInt32(Value); { SUB ESP,DWORD Value }
     end;
    end;
    LastOutputCodeValue:=locNone;
    PC:=PC+1;
   ...
   OPExit:begin
    Value:=Value-4;
    if Value>0 then begin
     EmitByte(0xc2); EmitInt16(Value); { RET Value }
    end else if Value=0 then begin
     EmitByte(0xc3); { RET }
    end else begin
     Error(145);
 ...
 { Size Of Code }
 PEEXECodeSize:=OutputCodeGetInt32(0x29)+(OutputCodeDataSize-PEEXECodeStart);
 OutputCodePutInt32(0x29,PEEXECodeSize);

 { Get section alignment }
 PEEXESectionAlignment:=OutputCodeGetInt32(0x45);
 ...
 OutputCodePutInt32(0x10d,PEEXESectionVirtualSize);

 { Calculate and patch image size }
 OutputCodePutInt32(0x5d,PEEXESectionVirtualSize+OutputCodeGetInt32(0x39));

 { Patch section raw size }
 OutputCodePutInt32(0x115,OutputCodeGetInt32(0x115)+(OutputCodeDataSize-PEEXECodeStart));
